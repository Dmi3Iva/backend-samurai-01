...
import type { UsersCoursesBindingViewModel } from "../../src/features/users-courses-bindings/models/UsersCoursesBindingViewModel";
import { expect } from "vitest";

const basePath = ROUTER_PATHS.usersCoursesBindings;

type HttpStatusCode = (typeof HTTP_CODES)[keyof typeof HTTP_CODES];

export const usersCoursesBindingsManager = {
  async createBinding({
    data,
    statusCode = HTTP_CODES.HTTP_STATUS_CREATED,
  }: {
    data: {
      userId: number;
      courseId: number;
  ...
  async getEntities({
    ...
    statusCode?: HttpStatusCode;
    expectedValue?: (UsersCoursesBindingViewModel | null)[];
  }) {
